...
def get_screener_symbol(company_name):
    ...
    data = r.json()
    if not data:
        raise ValueError("Company not found on Screener")
...
def get_year_column(df, target_year):
    try:
        ...
        for i, col_date in enumerate(df_cols):
            if not pd.isna(col_date) and col_date.year == target_year:
                return df.columns[i]
        return df.columns[0]
    except:
        return None
...
def iter_get(ticker,userticker,target_year):
    ...
    features = [
    "Total Revenue", "Net Income", "EBIT", "OCF", "FCF",
    "ROE", "ROCE", "Tax Rate", "Capex", "D&A",
    "Total Debt", "Shares",
    "Sentiment", "Leadership", "CapitalAllocation",
    "Governance", "Communication", "Stability", "MarketTrust","Revenue_YoY_Growth_%","Profit_YoY_Growth_%"
    ]
    feature = [
    "Current Price","Total Revenue", "Net Income", "EBIT", "OCF", "FCF",
    "ROE", "ROCE", "Tax Rate", "Capex", "D&A",
    "Total Debt", "Shares",
    "Sentiment", "Leadership", "CapitalAllocation",
    "Governance", "Communication", "Stability", "MarketTrust","Revenue_YoY_Growth_%","Profit_YoY_Growth_%"
    ]
    stock_data = {key: ta[key] for key in features}
    j=predict_growth(stock_data)
    growth_rate=j["CAGR_revenue"]
    stock_dat = {key: ta[key] for key in feature}
    run=run_dcf_analysis(stock_dat, growth_rate)
    data = {
        ...
        "Revenue_YoY_Growth_%": float(output["Revenue_YoY_Growth_%"]),
        "Profit_YoY_Growth_%": float(output["Profit_YoY_Growth_%"]),
        "CAGR_revenue":j["CAGR_revenue"],
        "CAGR_netincome":j["CAGR_netincome"],
        "Intrinsic":run["Intrinsic"],
        "Margin of safety":run["Margin of safety"],
        "OVER/UNDER":run["OVER/UNDER"],
    }
...
def main(query):
    ...
    file_path = "management_analysis.csv"
    file_path_1 = "COMPANY_sentiment.csv"
